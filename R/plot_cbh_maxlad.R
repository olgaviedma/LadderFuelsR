...
#' @export
get_plots_cbh_LAD <- function (LAD_profiles, cbh_metrics) {
    ...
    tryCatch({
      ...
      if (!is.na(min_y) && !is.na(max_y)) {
        ...
        tryCatch({
          ...
          if (!any(is.na(maxlad_CBH)) && !any(is.na(maxlad_Hdepth))) {
            if (maxlad_CBH != maxlad_Hdepth) {
              polygon_data_1 <- data.frame(x = c(maxlad_CBH, maxlad_CBH, maxlad_Hdepth, maxlad_Hdepth),
                                           y = c(min_y, max_y, max_y, min_y))
              bp2 <- bp2 +
                geom_polygon(data = polygon_data_1,
                             aes(x = x, y = y), fill = "dark green", alpha = 0.3)
            } else {
              line_data_1 <- data.frame(x = c(maxlad_CBH, maxlad_Hdepth),
                                        y = c(min_y, max_y))
              bp2 <- bp2 +
        ...
        Hcbh1_Hdptf1 <- as.numeric(as.character(df_effective1$maxlad_lad))
        ...
        if (any(!is.na(maxlad_CBH)) && any(!is.na(Hcbh1_Hdptf1a))) {

          y_1 = min_y
          bp2_annotations <- bp2_annotations + geom_text(data = data.frame(maxlad_CBH = maxlad_CBH, y_1 = min_y , Hcbh1_Hdptf1a = Hcbh1_Hdptf1a),
                                                         aes(x = maxlad_CBH,y = y_1, label = Hcbh1_Hdptf1a),
                                                         color = "black", hjust = -2.5, vjust = 0, size = 5)
          y_1 = max_y
          bp2_annotations <- bp2_annotations + geom_text(data = data.frame(maxlad_CBH = maxlad_CBH, y_1 = max_y , CBH1_label = CBH1_label),
                                                         aes(x = maxlad_CBH,y = y_1, label = CBH1_label),
